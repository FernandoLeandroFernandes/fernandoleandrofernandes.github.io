...
D(u,v) se dá pela seguinte fórmula:

Fórmula de D(u,v)

GamaH é o limite superior da atenuação do filtro e GamaL é o limite inferior. D0 é o raio do filtro.

Após a aplicação do filtro, calculou-se a Transformada Discreta Inversa de Fourier (Inverse Discrete Fourier Transform - IDFT), normalizou-se os elementos da imagem para valores de 0 a 1. 
...
// FILTER PARAMETERS
int K = 5;
int RADIUS_SLIDER = 80;
int GAMMA_LOW_SLIDER = 5;
int GAMMA_HIGH_SLIDER = 20;
...
void buildFilter(Mat& filter, int optimalHeight, int optimalWidth) {
  ...
  float H, W;
  float gain = (float) K/10;
  float gammaLow = (float) GAMMA_LOW_SLIDER/10;
  float gammaHigh = (float) GAMMA_HIGH_SLIDER/10;
  float radius = (float) RADIUS_SLIDER/10;
...
int main(int argc, char** argv) {
  ...
  do {
    ...
    sprintf( trackbar, "K x %d", MAX_K/10);
    cv::createTrackbar( trackbar, "Filtragem homomorfica", 
              &K, 
              MAX_K, 
              NULL);
...
Para a imagem acima, foram utilizados os seguintes parâmetros: Ganho(c): 0.5, Gama-low (Gl): 0.5, Gama-high(Gh): 0.8, Raio: 8.
...
void processCannyParameters(int, void*) {
  ...
  // Desenho dos pontos da borda da imagem para melhoria 
  // ... das características da imagem pontilhizada gerada
...
Observou-se que círculos com raio muito grande geravam um excesso artificial na marcação dos contornos da imagem. Assim, optou-se por reduzir o raio dos círculos pela metade assim como a frequência dos pontos desenhados, também pela metada.

Abaixo temos as imagens original e equalizada, resultado do processamento com a técnica pontilhista desenvolvida.
